...
public class CustomAuthenticationSuccess implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Authentication authentication) throws IOException, ServletException {
        ...
        for (GrantedAuthority grantedAuthority : authorities) {
            if (grantedAuthority.getAuthority().equals(RoleType.ADMIN)) {
                httpServletResponse.sendRedirect(httpServletRequest.getContextPath() + URL.DASHBOARD);
                break;
            } else {
                httpServletResponse.sendRedirect(httpServletRequest.getContextPath() + URL.HOME);
                break;
            }
        }
    }
